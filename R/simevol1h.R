...
##.ee.append("simpop",environment())
    ...
    if(check==FALSE){
        for(irad in 1:nrad){            
            ...
            n_next=deSolve::radau(func=pop_dynamics,time=mytime,y=en1,parms=parm,hini=hini,hmax=hmax,rootfun=rootfunc,rtol=rtol,atol=atol);
    ...
    if(check==TRUE){
        ...
        for(irad in 1:nrad){    
            mytime=10^seq(drad*irad,drad*(irad+1),,divrad);
            n_next=deSolve::radau(func=pop_dynamics,time=mytime,y=en1,parms=parm,hini=hini,hmax=hmax,rootfun=rootfunc,atol=atol,rtol=rtol);
...
plot_traj_line <-function(tree,xid,yid,p,mask=NULL){
    ##plines(a$tree,xid,yid,col=p$traj$col,pch=p$traj$pch,cex=p$traj$cex);
...
plot_func0 <- function(traj_line=FALSE){
    ...
    if(p$resi$bgid>0){
        bgid=p$resi$bgid;
        npal=length(p$pal);
        cmax=max(phe[[bgid]])+1e-10;
        cmin=min(phe[[bgid]])-1e-10;
        cid=as.integer((npal-1)*(phe[[bgid]]-cmin)/(cmax-cmin))+1;
        p$resi$bg=p$pal[cid];
##print(p$resi$bg);
    }
...
##'
##' The population dynamics triggerred by each mutant invasion is calculated with the implicit Runge-Kutta method (RDAU IIA) by means of R-package "deSolve". Implicit Runge-Kutta methods are known as A-stable, which is good for attaining the next population-dynamical equilibrium.
...
simevol <- function(phe=a$phe,en=a$en,## state values
                    ...
                    divrad=2,
                    bgid=-1,
                    param_desolve=list(hini=1e-4,hmax=1e9,rtol=1e-4,atol=1e-20)                      
                  ){
